...
from incar_video_pkg.utils import log_and_exit
...
class SaveToMp4(object):
    ...
    def subscribe_to_save_mp4(self, node):
        ...
        try:
            for camera_enum in self.camera_infos:
                ...
                if name not in self.mp4_subscription or self.mp4_subscription[name] is None:
                    self.mp4_subscription[name] = node.create_subscription(Image, topic_name, 
                                                                   lambda frame: self._subscribe_to_image_topic(frame, name), 10)
