...
from incar_video_pkg.constants import (PUBLISH_COMPRESSED_VIDEO_TOPIC, PUBLISH_SENSOR_TOPIC, PUBLISH_VIDEO_TOPIC, RECORDING_STATE_SERVICE_NAME, STATUS_TOPIC,
                                       Mp4Parameter, MAX_FRAMES_IN_QUEUE, QUEUE_WAIT_TIME, VIDEO_STATE_SRV, RecordingState)
...
class InCarVideoEditNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('racecar_name', 'DeepRacer', ParameterDescriptor(
        ...
        self.declare_parameter('save_to_mp4', True, ParameterDescriptor(
            type=ParameterType.PARAMETER_BOOL))
        self.declare_parameter('output_file_name', 'deepracer-{}.mp4',
                               ParameterDescriptor(type=ParameterType.PARAMETER_STRING))
        self.declare_parameter('fps', Mp4Parameter.FPS.value, ParameterDescriptor(
    ...
    def __enter__(self):
        ...
        self._state_service = self.create_service(RecordStateSrv,
                                                  RECORDING_STATE_SERVICE_NAME,
        ...
        self.get_logger().info('Node startup completed. Waitng to start recording.')
    ...
    def __exit__(self, ExcType, ExcValue, Traceback):
        ...
        if self._recording_state != RecordingState.Stopped:
            self._recording_state = RecordingState.Stopped
    ...
    def _start_recording(self):
        ...
        self._recording_state = RecordingState.Running
        ...
        if self._save_to_mp4:
            ...
            self.cv2_video_writer = cv2.VideoWriter(
                self._output_file_name_current, Mp4Parameter.FOURCC.value, self._fps, Mp4Parameter.FRAME_SIZE.value)
    ...
    def _stop_recording(self):
        ...
        """
        self.get_logger().info('Stopping the recording with {} frames in the queue.'.format(
    ...
    def _state_service_callback(self, req, res):
        ...
        Args:
            ...
            res (RecordingState.Response): Response object with error(int) flag to indicate
                                           if the service call was successful.

        Returns:
            RecordingState.Response: Response object with error(int) flag to indicate
                                     if the service call was successful.
        """
        if self._recording_state == RecordingState.Running and req.state == 0:
            self._stop_recording()
            res.error = 0

        elif self._recording_state == RecordingState.Running and req.state == 1:
            res.error = 1

        elif self._recording_state == RecordingState.Stopping and req.state == 1:
            res.error = 1

        elif self._recording_state == RecordingState.Stopping and req.state == 0:
            res.error = 1

        elif self._recording_state == RecordingState.Stopped and req.state == 0:
            res.error = 0

        elif self._recording_state == RecordingState.Stopped and req.state == 1:
    ...
    def _receive_camera_frame_callback(self, frame):
        ...
        if rclpy.ok():
            ...
            self.get_logger().debug(
                "Pushed {} frames to Frame Buffer.".format(self._edit_queue_pushed))
    ...
    def _buffer_timer_callback(self):
        ...
        try:
            ...
            if self._recording_state == RecordingState.Running:
                ...
                if (new_image_seen - self._last_image_seen > 1 and self._last_image_seen > 0):
                    self.get_logger().warn(
                        f"Image gap: { new_image_seen } of {new_image_seen - self._last_image_seen}")
            ...
            if self._recording_state == RecordingState.Running:
                self.get_logger().info(
                    "Input buffer is empty for {} seconds. Stopping.".format(QUEUE_WAIT_TIME))
    ...
    def _edit_frame_thread(self):
        ...
        while rclpy.ok() and self._recording_state != RecordingState.Stopped:
            frame_data = None
            try:
                ...
                if frame_data:
                    ...
                    self.get_logger().debug("Published {} frames. {} frames in queue.".format(
                        self._edited_frame_count, self._edit_queue.qsize()))

                    if self._publish_status:
                        self._status_pub.publish(StatusMsg(
                            state=1, published=self._edited_frame_count, queue=self._edit_queue.qsize()))

            except queue.Empty:
                self.get_logger().debug("Frame buffer is empty")

            if self._recording_state == RecordingState.Stopping and self._edit_queue.qsize() == 0:
                self._recording_state = RecordingState.Stopped
                self.get_logger().info("Stopped the recording after publishing {} frames. Queue empty.".format(
                    self._edited_frame_count))

        if self._save_to_mp4:
            self.cv2_video_writer.release()
            self.get_logger().info("Video written to {}.".format(self._output_file_name_current))
